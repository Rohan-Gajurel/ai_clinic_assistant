...
@section('title')
    <title>Edit Lab Group - TeleMed Admin</title>
@endsection
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-1 fw-semibold">Edit Lab Group</h4>
    ...
    <div class="col-lg-12">
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <form action="{{ route('lab-group.update', $group->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                            ...
                            <div id="tests-container" style="display: none; max-height: 300px; overflow-y: auto; border: 1px solid #dee2e6; border-radius: 0.375rem; padding: 10px;">
                                @foreach($tests as $test)
                                    <div class="form-check test-item" data-category="{{ $test->category_id }}" data-price="{{ $test->price ?? 0 }}">
                                        <input class="form-check-input test-checkbox" type="checkbox" name="test_ids[]" value="{{ $test->id }}" id="test{{ $test->id }}" {{ in_array($test->id, old('test_ids', $group->tests->pluck('id')->toArray())) ? 'checked' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" name="charge_amount" class="form-control" value="{{ old('charge_amount', $group->charge_amount) }}" required>
                        </div>
                    </div>

                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-plus-lg me-1"></i>Update Lab Group
                    </button>
