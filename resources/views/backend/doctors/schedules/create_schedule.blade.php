...
@section('title')
    <title>Create Schedules - MediNest Admin</title>
@endsection
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-1" style="color: var(--secondary-color); font-weight: 600;">Create New Schedule</h4>
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">
                    <i class="bi bi-key-fill me-2" style="color: var(--primary-color);"></i>Schedule Information
                </h5>
            </div>
            @if($errors->any())
            ...
            <div class="card-body">
                <form action="{{ route('schedules.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="doctor_id" id="doctor_id" class="form-select" required>
                            ...
                            @foreach($doctors as $u)
                                <option value="{{ $u->id }}" {{ old('doctor_id') == $u->id ? 'selected' : '' }}>{{ $u->user->name ?? 'N/A' }} ({{ $u->user->email ?? '' }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="days_of_week[]" id="days_of_week" class="form-select" multiple required>
                            @foreach(['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'] as $day)
                                <option value="{{ $day }}" {{ in_array($day, old('days_of_week', [])) ? 'selected' : '' }}>{{ $day }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="slot_duration" id="slot_duration" class="form-control" value="{{ old('slot_duration', 30) }}" min="1" required>
